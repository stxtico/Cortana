"""move (PROMPTS.md A24) - ordinary file move, write-capable, gated
(CLAUDE.md rule 4). Both source and destination resolve through
tools/_fs.py's existing write_whitelist_dirs whitelist, same reasoning as
copy.py - source has to be inside the whitelist too, not just the
destination.
"""

import shutil

from tools import _fs

REQUIRES_CONFIRMATION = True

_WRITE_KEY = "write_whitelist_dirs"


def spec() -> dict:
    return {
        "type": "function",
        "function": {
            "name": "move",
            "description": (
                "Move (or rename via a different path) a file or directory. Both the source "
                "and destination must be inside these directories (and their subdirectories): "
                f"{_fs.whitelist_description(_WRITE_KEY)}. Requires user confirmation before it "
                "actually runs."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "src": {"type": "string", "description": "Path to the file or directory to move, absolute or relative to the project root."},
                    "dst": {"type": "string", "description": "Destination path, absolute or relative to the project root."},
                },
                "required": ["src", "dst"],
            },
        },
    }


def describe(src: str, dst: str) -> str:
    return f"Move {src!r} to {dst!r}."


async def execute(src: str, dst: str) -> str:
    resolved_src = _fs.resolve_in_whitelist(src, _WRITE_KEY)
    resolved_dst = _fs.resolve_in_whitelist(dst, _WRITE_KEY)
    if not resolved_src.exists():
        return f"No such file or directory: {resolved_src}"

    resolved_dst.parent.mkdir(parents=True, exist_ok=True)
    shutil.move(str(resolved_src), str(resolved_dst))

    return f"Moved {resolved_src} to {resolved_dst}."
